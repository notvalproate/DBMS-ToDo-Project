...
app.post("/signup", async (req, res) => {
    const userExists = await DatabaseHandler.doesUsernameExist(
        req.body.username
    );
...
app.post("/usernameExists", async (req, res) => {
    res.json({
        exists: await DatabaseHandler.doesUsernameExist(req.body.username),
    });
});
...
app.post("/login", async (req, res) => {
    ...
    try {
        if (await bcrypt.compare(req.body.password, user.password)) {
            const jwtUser = await DatabaseHandler.getUserByUsernameSafe(
                req.body.username
            );
...
app.post("/getTodaysTodos", authenticateToken, async (req, res) => {
    const todaysTodos = await DatabaseHandler.getTasksForCurrentDate(
        req.user.username
    );
...
app.post("/addTodo", authenticateToken, async (req, res) => {
    const taskCreated = await DatabaseHandler.createTask(
        req.user.username,
        req.body.taskText
    );
...
app.post("/setTodo", authenticateToken, async (req, res) => {
    await DatabaseHandler.updateTaskCompletion(
        req.body.taskid,
...
app.get("/diary", authenticateToken, async (req, res) => {
    const diaryWritten = await DatabaseHandler.getIfDiaryWrittenToday(
        req.user.userid
    );
...
app.post("/addDiaryEntry", authenticateToken, async (req, res) => {
    await DatabaseHandler.insertDiaryEntry(
        req.user.userid,
        req.body.content,
        parseInt(req.body.mood)
    );
...
app.post("/getPast7Diaries", authenticateToken, async (req, res) => {
    const diaries = await DatabaseHandler.getDiariesByUserIdLast7Days(
        req.user.userid
    );
...
app.post("/createMessage", authenticateToken, async (req, res) => {
    await DatabaseHandler.insertMessageEntry(
        req.user.userid,
        req.body.message,
        req.body.date
    );
...
app.post("/getLogs", authenticateToken, async (req, res) => {
    const todos = await DatabaseHandler.getTasksByDate(
        req.user.userid,
        req.body.date
    );
    const diary = await DatabaseHandler.getDiaryByDate(
        req.user.userid,
        req.body.date
    );
    const messages = await DatabaseHandler.getMessagesByDate(
        req.user.userid,
        req.body.date
    );
...
app.post("/getData", authenticateToken, async (req, res) => {
    ...
    const averageMoodWeek = await DatabaseHandler.getAverageMoodLast7Days(
        req.user.userid
    );

    const totalLastWeek = await DatabaseHandler.getTotalTasksInLast7Days(
        req.user.userid
    );
    const totalCompletedLastWeek =
        await DatabaseHandler.getTotalTasksCompletedInLast7Days(
            req.user.userid
        );
    const productivityWeek = (totalCompletedLastWeek * 100) / totalLastWeek;

    const averageMoodMonth = await DatabaseHandler.getAverageMoodLast30Days(
        req.user.userid
    );

    const totalLastMonth = await DatabaseHandler.getTotalTasksInLast30Days(
        req.user.userid
    );
    const totalCompletedLastMonth =
        await DatabaseHandler.getTotalTasksCompletedInLast30Days(
            req.user.userid
        );
    ...
    const graphTotalTasks7 = await DatabaseHandler.getTotalTasksSumForLast7Days(
        req.user.userid
    );
    const graphCompletedTasks7 =
        await DatabaseHandler.getCompletedTasksSumForLast7Days(req.user.userid);

    const graphTotalTasks30 =
        await DatabaseHandler.getTotalTasksSumForLast30Days(req.user.userid);
    const graphCompletedTasks30 =
        await DatabaseHandler.getCompletedTasksSumForLast30Days(
            req.user.userid
        );

    const moodPastWeek = await DatabaseHandler.getMoodPastWeek(req.user.userid);
    const moodPastMonth = await DatabaseHandler.getMoodPastMonth(
        req.user.userid
    );
